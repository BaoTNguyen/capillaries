...
- **A floor, not just a ranker.** Retrieval can return nothing. The top rerank score has to clear `MIN_CONFIDENCE` (0.8 by default, overridable with `CAPILLARIES_MIN_CONFIDENCE`) before a prompt comes back, so an agent asking about something your corpus doesn't cover gets `mode="none"` instead of the nearest irrelevant match. Every retrieval surface delegates to the same `find()` path, so that decision cannot drift.
