"""
MCP Server for Prompt Flow.

Provides 4 MCP tools for agent integration:
- prompt_flow_find: Find the best prompt or skill
- prompt_flow_execute_step: Execute next step in a skill
- prompt_flow_feedback: Report outcome
- prompt_flow_catalog: Browse available capabilities

Usage:
    python -m prompt_flow.mcp_server
...
from prompt_flow.agent.api import (
    RouteRequest,
    StepRequest,
    FeedbackRequest,
    _get_router,
...
try:
    ...
    async def prompt_flow_execute_step(
        ...
        Call this after prompt_flow_find returns a skill (mode=skill).
    ...
    async def prompt_flow_feedback(
        ...
        Call this after using a prompt from prompt_flow_find.
    ...
    def prompt_flow_find(situation: str, stage: str = None, domain: list = None, complexity: int = None, prefer: str = "auto") -> dict:
        raise ImportError("MCP SDK not installed. Run: pip install mcp")

    def prompt_flow_execute_step(session_id: str, step_order: int, previous_output: str = None, variables: dict = None, skip_reason: str = None) -> dict:
        raise ImportError("MCP SDK not installed. Run: pip install mcp")

    def prompt_flow_feedback(trace_id: str, outcome: str, quality_score: float = None, failure_step: int = None, notes: str = None) -> dict:
        raise ImportError("MCP SDK not installed. Run: pip install mcp")

    def prompt_flow_catalog(view: str = "overview", domain_filter: str = None) -> dict:
        raise ImportError("MCP SDK not installed. Run: pip install mcp")