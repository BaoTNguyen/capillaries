...
Usage:
    python -m capillaries.search.eval                        # default queries, top 5
    python -m capillaries.search.eval --top-k 3              # fewer singles per query
    python -m capillaries.search.eval --out report.txt       # custom output path
    python -m capillaries.search.eval --query "your query"   # single query
"""
...
def _render_skill(lines: list, match) -> None:
    ...
    for step in match.steps:
        lines.append(
            f"    Step {step['step_order']}  {step['prompt_id']}"
        )
...
def _render_suggestion(lines: list, steps: list) -> None:
    ...
    if not steps:
        lines.append("  No steps produced a match above the weak threshold.")
        lines.append("")
        return

    lines.append(f"  SUGGESTED: {len(steps)}-step skill")
    lines.append("")
    for i, step in enumerate(steps, 1):
        weak = step.rerank_score < WEAK_MATCH_THRESHOLD
        flag = "  ⚠ WEAK" if weak else ""
        lines.append(
            f"    Step {i}  {step.prompt_id}{flag}"
        )
        lines.append(
...
async def run_report(
    ...
    lines.append(f"  CUSTOM_SKILL: Neither a single prompt nor an existing skill "
                 "fit — review the suggested assembly.")
    lines.append(f"  ⚠ WEAK = rerank < {threshold} (poor fit for that step).")
    lines.append("")
